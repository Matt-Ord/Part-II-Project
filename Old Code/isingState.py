...
class IsingState():
    dimension  = None
    solverType = None
    def __init__(self, state, spinStateSolver, previousState = None, neighbours = None, nextNeighbours = None):
        ...
        self.currentSpins = None                    #Array for intermediate calculations
        ...
        if(nextNeighbours is None):
            self._calculateAllNextNeighbours()
        else:
            self.nextNeighbours = nextNeighbours
    ...
    def getMagnetisation(self):
        return (2 * self.getNumberOfSpinUp() - self.getNumberOfSpins()) / self.getNumberOfSpins()  # Spin up - spin down, saves calling np.count_Nonzero twice
    ...
    def getAllInitialParameters(self):            #Get parameter for all positions in the lattuice
        return np.tile(False, reps = self.getDimensions())
    
    def getAllCurrentParameters(self):
        return self.getAllInitialParameters()
    
    def getAllFields(self):                 #Defaults to constant field everywhere
        return np.tile(self.getAverageField(), reps = self.getDimensions())
    ...
    def getField(self, indices):
        return np.take(self.getAllFields(), indices)
    ...
    def getIndexOfCoordinate(self, coordinates):
        return np.ravel_multi_index(coordinates, dims = self.getDimensions(), mode = "wrap")
    ...
    def getTotalEnergyOf(self, indices):
        return np.sum(self.getEnergyOf(self.getInitialLocalState(indices)))
    ...
    def getAverageField(self):
        return np.average(self.getAllFields)
    ...
    def getRandomCoordinate(self, size = None):
        return np.unravel_index(self.getRandomIndex(size = size), self.getDimensions())

    def getStateAsArray(self):
        return np.array(self.getAllInitialSpins())
    
    def getAllEnergyAsArray(self):
        return np.array(self.getEnergies())
    ...
    def _calculateAllNeighbours(self):
        allIndices = self.getAllIndex().reshape(self.getDimensions())
        neighbours = []
        for axis in range(self.getDimension()):
    ...
    def _seperateNeighbouringIndicies(self, indices):
        ...
        for pos in range(indices.size):
            ...
            #The neighbourhood surrounding an index is all points which will be effected if index is updated
    ...
    def _calculateNextState(self):
        self.currentSpins = np.array(self.getAllInitialSpins()) 
        
        regions = self._getRegionsToUpdate()
        for r in regions:
            self._updateSpinRegion(r)
    ...
    def _getNextKwargs(self):
        return {'neighbours' : self.neighbours,
                'nextNeighbours' : self.nextNeighbours,
                'state' : self.currentSpins,
    ...
    @classmethod
    def generateFromArray(cls, array, temperature, solverKwargs = {}, **kwargs):
        ...
        solver = cls.solverType(temperature, cls.dimension, **solverKwargs)

        return cls(state = state, spinStateSolver = solver, previousState = None, **kwargs)
...
class ForgetfulIsingState(IsingState):
    ...
    def _getNextState(self):
        ...
        self.nextState = None              # Forgets about the next state
        return nextState
    
    def _getNextKwargs(self):
        kwargs = super()._getNextKwargs()
        kwargs['previousState'] = None     # Forgets about the previous state
        return kwargs
...
class IsingState2D(IsingState):
    ...
    def getStateAsImage(self):
        return plt.imshow(self.getAllInitialSpins(), aspect='equal', cmap=plt.cm.gray, interpolation='nearest', animated = True, norm=plt.Normalize(0, 1))
    
class IsingState3D(IsingState):
    dimension = 3
    
class InterpolatedIsingState(IsingState):
...
class VectorisedIsingState(IsingState):
    ...
    def _getInterpolatedSpins(self, N):
        ...
        if(self.currentSpins is None): #If not already calculated
            self._calculateNextState()
...
class VectorisedRandomIsingState(VectorisedIsingState, RandomIsingState):
    ...
    def _generateOrderOfCalculation(self):
        ...
        randomCoordinateIndex = self.getRandomIndex(size = self.getNumberOfSpins())
...
class FastSystematicIsingState(SystematicIsingState):
    ...
    def __init__(self, numberOfSplits = None):
        self.numberOfSplits = numberOfSplits if numberOfSplits != None else self.getDimensions()[0]
...
class TemperatureVaryingIsingState(IsingState):
    ...
    def getTemperature(self):
        return self.getInitialParameter(0)
    
    def getAllInitialParameters(self):
        return np.tile(self.spinStateSolver.getParameter(self.position), reps = self.getDimensions())
    ...
    def _discretiseTemperatures(temperatures, possibleT):
        ss = np.searchsorted(possibleT, temperatures)
        tempBelow = possibleT[ss - 1]
        tempAbove = possibleT[np.minimum(len(possibleT) - 1, ss)]
        discretisedTemperatures = np.where(np.fabs(temperatures - tempBelow) < np.fabs(temperatures - tempAbove), tempBelow, tempAbove)
        return discretisedTemperatures
    ...
    @classmethod
    def generateOscillatingStateFromArray(cls, startTemp, endTemp, numberOfSteps, resolution = 20, **kwargs):
        if(startTemp < endTemp):
            ...
            possibleT = np.linspace(endTemp,startTemp, num = resolution)
        
        discretisedTemperatures = cls._discretiseTemperatures(temperatures, possibleT)
        return cls.generateFromArray(temperatures = discretisedTemperatures, **kwargs)
    
    @classmethod
    def generateRandomOscillatingState(cls, startTemp, endTemp, numberOfSteps, resolution = 20, **kwargs):
        if(startTemp < endTemp):
            temperatures = np.linspace(startTemp, endTemp, num = numberOfSteps)
            possibleT = np.linspace(startTemp, endTemp, num = resolution)
        else:
        ...
        discretisedTemperatures = cls._discretiseTemperatures(temperatures, possibleT)
        return cls.generateRandomState(temperatures = discretisedTemperatures, **kwargs)
...
class MagnetisedIsingState(IsingState):
    ...
    def getAverageField(self):
        return self.spinStateSolver.getParameter(self.position)
    ...
    @classmethod
    def generateOscillatingStateFromArray(cls, amplitude, numberOfSteps, resolution = 20, **kwargs):
        fields    = np.array([amplitude  * np.cos(x * 2 * np.pi / numberOfSteps) for x in range(numberOfSteps)])
        possibleH = np.linspace(-amplitude, amplitude, num = resolution)
        
        discretisedfields = possibleH[abs(fields[None, :] - possibleH[:, None]).argmin(axis=0)]
        return cls.generateFromArray(fields = discretisedfields, **kwargs) 
        
    @classmethod
    def generateRandomOscillatingState(cls, amplitude, numberOfSteps, resolution = 20, **kwargs):
        fields    = np.array([amplitude  * np.cos(x * 2 * np.pi / numberOfSteps) for x in range(numberOfSteps)])
        possibleH = np.linspace(-amplitude, amplitude, num = resolution)
        
        discretisedfields = possibleH[abs(fields[None, :] - possibleH[:, None]).argmin(axis=0)]
        return cls.generateRandomState(fields = discretisedfields, **kwargs) 
...
class RandomNearestNeighbourForgetfulIsingState1D(ForgetfulIsingState, RandomNearestNeighbourIsingState2D):
...
class RandomNearestNeighbourForgetfulIsingState3D(ForgetfulIsingState, RandomNearestNeighbourIsingState2D):
...
class OldSystematicIsingState(IsingState):
    ...
    def getStateNumber(self, N):
        if(N < self.getNumberOfSpins()):
            return self.getIntermediateState(N)
        return self._getNextState().getStateNumber(N - self.getNumberOfSpins())
    ...
    def getIntermediateStatesArray(self, N):
        return self._getInterpolatedSpins(N)

    
    def _getInterpolatedSpins(self, N):
        self._calculateNextState()
        interpolatedSpins = np.copy(self.getAllInitialSpins())
        interpolatedSpins.flat[:N]  = self.getAllCurrentSpins().flat[:N]

        return interpolatedSpins
    
    def _calculateNextState(self):
        #Generates the next state by updating all coordinates
        if(self.currentSpins is None):
            self.currentSpins = np.array(self.getAllInitialSpins()) 
            ...
            for s in self.neighbourhoodSlices:
                ...
                isNeighbourhoodFlipped, neighbourhoodNextSpins = self.spinStateSolver.getNextSpinStates(neighbourhoodLocalStates)
                self.currentSpins[s] = neighbourhoodNextSpins                 #[s] - only neighbourhood slice is updated
'''            

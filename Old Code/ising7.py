...
class SystematicIsingState(IsingState):
    ...
    def getNextState(self):
        ...
        localStates = self.getAllInitialLocalStates()
        _, self.currentState = self.spinStateSolver.getNextSpinStates(localStates)
        
        self._setNextState()
        return self.nextState
    
class ImprovedSystematicIsingState(SystematicIsingState):
    #Improves the systematic approach by only updating Neighbourhoods which don't share nearest
    #neighbours - reducing the appearance of sudden jumps + preventing oscillations
    
    #Classes Implementing the improved method should provide a list conataining
    #the required slicing rules of each neighbourhood
...
class SystematicNearestNeighbourIsingState2D(IsingState2D, ImprovedSystematicIsingState, NearestNeighbourIsingState):
    neighbourhoodSlices = [np.s_[0::2,0::2], np.s_[1::2,0::2], np.s_[2::2,0::2],
                           np.s_[0::2,1::2], np.s_[1::2,1::2], np.s_[2::2,1::2],
                           np.s_[0::2,2::2], np.s_[1::2,2::2], np.s_[2::2,2::2]] #Total of 9 adjoint neighbourhoods in the case of 2D nearest neighbour ising states
...
if __name__ == '__main__':
    ...
    sim = NearestNeighbourIsingSimulation2D(100000, 200, 200)
    plotter = IsingSimulationPlotter2D(sim)
    #sim.getStateNumber(30000)
    start = time.time()
    #sim.getStateNumber(300000)
    sim.getStateNumber(200)
    plotter.animateFirstNStates(200)
    
    end = time.time()
    plotter.plotFirstNMagnetisations(200)
    print(end-start)
    start = time.time()
